...
-- Culture functions 
-------------------------------------------------
function GetPlotCulture( plotKey )
	local plot = GetPlotFromKey ( plotKey )
	local plotCulture = {}
	for playerID = 0, REALLY_MAX_PLAYERS do -- including fake players (like separatists)
		if plot:GetCulture(playerID) > 0 then
...
function GetPlotCulturePercent( plotKey )
	...
	if (totalCulture > 0) then
		for playerID = 0, REALLY_MAX_PLAYERS do -- including fake players (like separatists)
			if plot:GetCulture(playerID) > 0 then
				table.insert (plotCulturePercent, { ID = playerID, Value = plot:GetCulturePercent(playerID) } )	
			end
		end
			...
			if airRatio + landRatio + seaRatio + roadRatio + railRatio > 0 then -- at least one value has changed
				...
				if distFactor > 0 then
					...
					if distFactor >= 1 then -- value under 1 generate negative handicap...
						...
						local separatistValue = plot:GetCulture(SEPARATIST_PLAYER)
						local turnFounded = city:GetGameTurnFounded()
						local turn = Game.GetGameTurn()
						if turn > turnFounded + TURN_BEFORE_CHECK_INDEPENDENCE then

							for playerID = 0, GameDefines.MAX_PLAYERS do -- only normal players
								local toConvert = Round(plot:GetCulture(playerID) * convertRatio  /100)
								Dprint ("         - converting " .. toConvert .. " of player " .. playerID .. " (culture=".. plot:GetCulture(playerID) ..") to SEPARATIST (" .. separatistValue .. ")", bDebug)

								plot:ChangeCulture(playerID, - toConvert )
								plot:ChangeCulture(SEPARATIST_PLAYER, toConvert )
								bMapChanged = true
...
function ReserveCS()
	...
	local numCS = math.max(0, PreGame.GetNumMinorCivs() - RESERVED_CITY_STATES) -- can't use custom setup screen data in MP !
...
function GetCultureGroups(iPlayer, bDebug)
	...
	for city in player:Cities() do
		...
		for playerID = 0, GameDefines.MAX_PLAYERS do
			if plot:GetCulture(playerID) > 0 then
				if cultureGroups[playerID] then
					cultureGroups[playerID] = cultureGroups[playerID] + plot:GetCulture(playerID)
					Dprint ("    - adding " .. plot:GetCulture(playerID) .. " for player " .. playerID .. ", total is " .. cultureGroups[playerID], bDebug)
					totalCulture = totalCulture + plot:GetCulture(playerID)
				else
					cultureGroups[playerID] = plot:GetCulture(playerID)
					Dprint ("    - initializing " .. plot:GetCulture(playerID) .. " for player " .. playerID, bDebug)
					totalCulture = totalCulture + plot:GetCulture(playerID)
				end
			end
		end
		...
		-- Separatist group
		if plot:GetCulture(SEPARATIST_PLAYER) > 0 then
			if cultureGroups[SEPARATIST_PLAYER] then
				cultureGroups[SEPARATIST_PLAYER] = cultureGroups[SEPARATIST_PLAYER] + plot:GetCulture(SEPARATIST_PLAYER)
				Dprint ("    - adding " .. plot:GetCulture(SEPARATIST_PLAYER) .. " for separatist, total is " .. cultureGroups[SEPARATIST_PLAYER], bDebug)
				totalCulture = totalCulture + plot:GetCulture(SEPARATIST_PLAYER)
			else
				cultureGroups[SEPARATIST_PLAYER] = plot:GetCulture(SEPARATIST_PLAYER)
				Dprint ("    - initializing " .. plot:GetCulture(SEPARATIST_PLAYER) .. " for separatists", bDebug)
				totalCulture = totalCulture + plot:GetCulture(SEPARATIST_PLAYER)
			end
		end
	...
	for id, value in pairs(cultureGroups) do
		table.insert (sortedCultureGroups, { Type = id, Value = value } )
	end
...
function UpdateRebels(iPlayer)
	...
	for city in player:Cities() do
		...
		local civsCulture = GetPlotCulturePercent( plotKey )
		...
		for i = 1, #civsCulture do
			if (i <= MAX_REBELS_GROUPS) and (civsCulture[i].Value > MIN_RATIO_REBELS_SPAWN) then
				...
				local cultureTypeValue =  cityPlot:GetCulture(cultureType)
				if cultureTypeValue >= MIN_CULTURE_REBELS_SPAWN then 
					Dprint ("    -  player " .. cultureType .. " has enought strength to rebel, check relation...", bDebug)
					local culturePercentValue = civsCulture[i].Value
					if not (cultureRelations[iPlayer][cultureType]) then
						Dprint ("WARNING : No entry in culture relation table for player " .. cultureType .. " with player " .. iPlayer  )
					else
						local relation = cultureRelations[iPlayer][cultureType]
						if relation < MIN_RELATION_BEFORE_REBELLION then
							Dprint ("    - Bad relation with tile owner for player "..cultureType..", check for rebellion spawning...", bDebug)
							local citySize = city:GetPopulation()
							local randNum = Game.Rand( 100, "check for rebellion" )
							...
							local cityHappiness = GetCityHappiness(city, cultureRelations)
							...
							local revoltChance = Game.Rand( 100, "check for revolt" )
							...
							local rebellionChance = Game.Rand( 100, "check for rebels spawning" )
							...
							local revolutionChance = Game.Rand( 100, "check for revolution" )
...
function GetCityRebellion(city, cultureRelations, bDebug)
		...
		if (i <= MAX_REBELS_GROUPS) and (civsCulture[i].Value > MIN_RATIO_REBELS_SPAWN) then
			...
			elseif not (cultureRelations[iPlayer][cultureType]) then
				...
				if relation < MIN_RELATION_BEFORE_REBELLION then
					...
					Dprint ("   - Bad relation with tile owner for "..GetCultureTypeAdj(cultureType)..", check for rebellion spawning...", bDebug)
					local militaryPresence, martialLawValue = GetNumMilitaryUnitAround(city)
					...
					local cityHappiness = GetCityHappiness(city, cultureRelations, bDebug)
					...
					--if rebellion > 0 then
						...
						-- maybe this is not needed anymore, calculation being based on global city happiness, not from specific culture relation...
...
function GetCityHappiness(city, cultureRelations, bDebug)
...
function SpawnRebels( city, iPlayer, cultureType, numRebels)
	...
	local player = Players[iPlayer]
	Dprint ("            - Spawning "..GetCultureTypeAdj(cultureType).." Rebels against " .. player:GetName(), bDebug)
...
function FreeReservedCS()
	...
	for iPlotLoop = 0, Map.GetNumPlots()-1, 1 do
		...
		for j, id in pairs(civToRemove) do
			if plot:GetCulture(id) > 0 then
				Dprint (" - found culture to remove on plot " .. tostring(plotKey) .." : " .. GetCultureTypeAdj(id), bDebug)	
				plot:SetCulture(id, 0)
			end
		end
	end
...
function RevolutionInCity( city, iPlayer, cultureType)
	...
	for i = 0, city:Plot():GetNumUnits() - 1, 1 do
		...
		if unit and (unit:GetOwner() == iPlayer) then
			if unit:IsCombatUnit() then
				local damage = UNIT_DAMAGE_REVOLUTION + Game.Rand( UNIT_DAMAGE_REVOLUTION_VAR, "Unit damage from revolution" )
	...
	if not rebelID then
		if #freeSlots > 0 then
			...
			Dprint ("              - WARNING : No free slot !!!", bDebug)
			local damage = CITY_DAMAGE_REVOLUTION + Game.Rand(CITY_DAMAGE_REVOLUTION_VAR, "city damage from revolution" )
			city:ChangeDamage(damage)
			return false -- can't set rebelID
		end
	end
	...
	city:ChangeDamage(Game.Rand(CITY_DAMAGE_REVOLUTION_VAR, "city damage from revolution" ))
...
function GetPlotRelationHelpString(plot, cultureRelations)
	
	local plotKey = GetPlotKey ( plot )
	local plotCulture = GetPlotCulture( plotKey )
...
function GetRelationChange(iPlayer, cultureType, bDebug)
	...
	data.cultureType = GetCivTypeFromPlayer(cultureType)
	data.player = Players[iPlayer]
	data.teamID = data.player:GetTeam()
	data.playerCultureID = GetCivTypeFromPlayer(iPlayer)
	if cultureType ~= SEPARATIST_PLAYER then
		data.otherPlayer = Players[ cultureType ]
		...
		if city then
			if data.CountDown < 1 then -- now we can make a choice...
				...
				if rebelID == data.RebelID then
					...
					for i = 1, #civsCulture do
						local cultureID = civsCulture[i].ID
						local cultureType = GetCivTypeFromPlayer(cultureID)
						Dprint ("      - checking " ..GetCultureTypeAdj(cultureType), bDebug)
						if cultureType ~= SEPARATIST_TYPE and cultureType ~= rebelType then
							...
							if player then -- don't check for alive here, could be great to revive dead player !
								...
								local cultureStrength = cityPlot:GetCulture(cultureID)
					...
					local rebelValue = cityPlot:GetCulture( rebelID )
					Dprint ("   - rebel culture value = " ..tostring(rebelValue), bDebug)					

					local separatistValue = cityPlot:GetCulture( SEPARATIST_PLAYER )
					...
					if bestChoice then
						...
						local iPlayer = GetPlayerIDFromCivType (bestChoice)
						local player = Players[iPlayer]
						
						-- Update culture map
						cityPlot:ChangeCulture( iPlayer, rebelValue + separatistValue )
						cityPlot:ChangeCulture( rebelID, - rebelValue )
						cityPlot:ChangeCulture( SEPARATIST_PLAYER, - separatistValue )
					...
					elseif (REVOLUTION_SPAWN_NEW_CS) and (Game.Rand( 100, "declaration of independance after revolution" ) < NEW_CS_PERCENT_CHANCE ) then -- can spawn new CS ?
						Dprint ("   - The revolutionnary comite is looking at the possibility of a declaration of independance...", bDebug)

						local freeSlots = GetFreeSlotsAndRebelID(reservedCS, masterID, rebelID)
						if #freeSlots > MIN_FREE_SLOTS_FOR_NEW_CS then
							local newCivID = GetRebelIDForArtStyle(freeSlots, masterID, rebelID)
							if newCivID then
								...
								-- Update culture map
								cityPlot:ChangeCulture( newCivID, rebelValue + separatistValue )
								cityPlot:ChangeCulture( rebelID, - rebelValue )
								cityPlot:ChangeCulture( SEPARATIST_PLAYER, - separatistValue )
					...
					elseif data.CountDown < -REVOLUTION_FORCE_CHOICE_COUNTDOWN then -- must make a choice now !
						...
						if closeiPlayer then
							...
							-- Update culture map
							cityPlot:ChangeCulture( closeiPlayer, rebelValue + separatistValue )
							cityPlot:ChangeCulture( rebelID, - rebelValue )
							cityPlot:ChangeCulture( SEPARATIST_PLAYER, - separatistValue )
...
function GetRebelIDForArtStyle(freeSlots, iPlayer, rebelCultureType)
	...
	local playerCiv = GetCivIDFromPlayerID (iPlayer)
	local rebelCultureType = playerCiv
	if rebelCultureType ~= SEPARATIST_PLAYER then
		rebelCultureType = GetCivIDFromPlayerID (rebelCultureType) -- rebelCultureType is now a playerID...
	end
...
function CheckForDeadRebel (iAttackingPlayer, iAttackingUnit, attackerDamage, attackerFinalDamage, attackerMaxHP, iDefendingPlayer, iDefendingUnit, defenderDamage, defenderFinalDamage, defenderMaxHP, iInterceptingPlayer, iInterceptingUnit, interceptorDamage, plotX, plotY)
	...
	if pAttackingPlayer then -- In case the combat was aborted...
		...
		if pDefendingUnit and defenderHealth <= 0 then
			if IsRebellingAgainst(iDefendingPlayer, iAttackingPlayer) then
				Dprint ("------------------ ", bDebug)
				Dprint ("Matting rebellion !", bDebug)

				local rebelType = GetRebelType(iDefendingPlayer)
				
				Dprint ("Rebel type = " .. rebelType, bDebug)
				
				Dprint ("Rebel type = " .. rebelType, bDebug)
				Dprint ("Attacking = " .. iAttackingPlayer, bDebug)
				Dprint ("Defending = " .. iDefendingPlayer, bDebug)
