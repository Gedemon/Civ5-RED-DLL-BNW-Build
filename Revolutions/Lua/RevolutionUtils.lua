...
function GetCivIDFromPlayerID (iPlayer, bReportError)
	if (iPlayer ~= -1) then
...
function GetCivTypeFromPlayer (iPlayer, bReportError)
	...
	-- first return fake players (like separatists)
	if iPlayer == SEPARATIST_PLAYER then
		return SEPARATIST_TYPE
	end
...
function HideMinorWarButton( popupInfo )
	...
	if g_MinorProtector then
		local protectorList = g_MinorProtector[GetCivIDFromPlayerID (minoriPlayer)]
...
function GetRebelType(minoriPlayer)
	local reservedCS = LoadData ("ReservedCS")
	if reservedCS[minoriPlayer] and reservedCS[minoriPlayer].Type then
		return reservedCS[minoriPlayer].Type
	else
		return nil
	end
end
